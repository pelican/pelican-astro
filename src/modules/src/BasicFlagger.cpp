 ...
 *
 * @param[in,out] visibilityData The hash of data blobs required to run the
 *      pipeline module. The module updates the contents of the data blobs in
 *      the hash as required.
 */
void BasicFlagger::run(VisibilityData* visibilityData, FlagTable* flagTable)
{
    ...
    unsigned nAntennas = visibilityData->nAntennas();
    unsigned nChannels = visibilityData->nChannels();
    unsigned nPols = visibilityData->nPolarisations();
 ...
 */
void BasicFlagger::_flagAutocorrelations(const VisibilityData* visData,
        ...
        const Real maxFraction, FlagTable* flagTable)
{
    ...
    for (unsigned p = 0; p < nPols; ++p) {
        for (unsigned c = 0; c < nChannels; ++c) {
            ...
            const Complex* mptr = visData->ptr(c, p);

            // Get current median autocorrelation power
            Real medianPower = abs(medians[c + nChannels * p]);

            // Check the diagonals
            for (unsigned a = 0; a < nAntennas; ++a) {
                Real antennaPower = abs(mptr[a + nAntennas * a]);
 ...
 */
void BasicFlagger::_getAutocorrelations(const VisibilityData* visData, Complex* aptr)
{
    // Get the dimension sizes
    const unsigned nAntennas = visData->nAntennas();
    const unsigned nChannels = visData->nChannels();
 ...
 */
void BasicFlagger::_getMedians(unsigned nAntennas, unsigned nChannels,
        unsigned nPols, const Complex* aptr, Complex* mptr)
{
    // Get the dimension sizes
    const unsigned nAntennasChannels = nAntennas * nChannels;
    ...
    Complex* autocorr = (Complex*)malloc(size * sizeof(Complex));
    memcpy(autocorr, aptr, size * sizeof(Complex));
 ...
 */
void BasicFlagger::_moveBadAntennas(VisibilityData* visData, FlagTable* flagTable)
{
    // Get the dimension sizes
    const unsigned nAntennas = visData->nAntennas();
    const unsigned nChannels = visData->nChannels();
    ...
    for (unsigned p = 0; p < nPols; ++p) {
        for (unsigned c = 0; c < nChannels; ++c) {
            ...
            // that when we swap indices (starting from the highest bad antenna),
            // we don't interfere with anything else. There's no way round this!
